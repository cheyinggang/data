...
setwd("D:\\bioinformatics\\3.COVID-19\\0.write\\2.paper_source\\F_3\\6.C")

rt=read.table("UBI_MF_pathways.txt",sep = "\t",header = T,row.names = 1)
data=rt
...
for_label <- df %>% 
  filter(abs(logFC) >0.7& adj.P.Val< 0.05)
...
ggsave("Figure_C.pdf", height = 12, width = 15)
