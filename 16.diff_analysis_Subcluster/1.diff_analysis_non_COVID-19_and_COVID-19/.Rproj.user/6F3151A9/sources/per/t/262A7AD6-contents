...
rm(list = ls())
library(limma)
rt=read.table("COVID_19_geneMatrix.txt",sep = "\t")
rt=as.matrix(rt)
rownames(rt)=rt[,1]
colnames(rt)=rt[1,]
data=rt[2:nrow(rt),2:ncol(rt)]

group  <- c(rep("non_COVID_19",26),rep("COVID_19",100)) 
group <- factor(group,levels = c("COVID_19","non_COVID_19"))
...
p1
ggsave("Figure_A.pdf",width = 8,height = 6)
